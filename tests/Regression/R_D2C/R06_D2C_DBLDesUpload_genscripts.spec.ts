 ...
 * Validate DB Level Design upload with options generateReports, generateMeta, generateScripts.
 */
...
test.describe('@D2CRegression DB Level Design Upload Generate Scripts Validations', () => {
  test('R06_D2C_DBLDesUpload_genscripts', async ({ page }) => {
    ...
    clearPotFolder('D2C', 'R06_D2C_DBLDesUpload_genscripts');
    ...
    await capturePotStep(page, 'D2C', 'R06_D2C_DBLDesUpload_genscripts', 2, 'DB Level Design table loaded');
    ...
    await capturePotStep(page, 'D2C', 'R06_D2C_DBLDesUpload_genscripts', 3, 'Upload completed');
    ...
    await generatePotDocument('D2C', 'R06_D2C_DBLDesUpload_genscripts');
  });
});
